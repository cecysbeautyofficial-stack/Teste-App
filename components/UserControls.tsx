...
import { SunIcon, MoonIcon, ChartBarIcon, CogIcon, LogoutIcon } from './Icons';
        ...
        <div className="flex items-center gap-3 sm:gap-4">
            ...
            {currentUser ? (
                <div className="relative" ref={userMenuRef}>
                    <button onClick={() => setIsUserMenuOpen(prev => !prev)} className="flex items-center gap-2 focus:outline-none">
                        <img src={currentUser.avatarUrl || `https://i.pravatar.cc/150?u=${currentUser.id}`} alt={currentUser.name} className="h-10 w-10 rounded-full border-2 border-white/50 dark:border-white/10 object-cover shadow-sm" />
                    </button>
                     {isUserMenuOpen && (
            ...
            <div className="sm:hidden">
              {currentUser ? null : (
                <button onClick={onLogin} className="font-semibold text-gray-600 dark:text-gray-300 p-2">
                  Entrar
                </button>
              )}
            </div>
